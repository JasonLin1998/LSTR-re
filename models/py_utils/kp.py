...
class kp(nn.Module):
    def __init__(self,
        ...
        self.class_embed    = nn.Linear(hidden_dim, num_cls + 1)  # pred_logits
        self.specific_embed = MLP(hidden_dim, hidden_dim, lsp_dim - 4, mlp_layers)  # pred_curves
        self.shared_embed   = MLP(hidden_dim, hidden_dim, 4, mlp_layers)
        if self.curve_predictor:
            self.curve_embed = MLP(hidden_dim, hidden_dim, 1, 2)
    ...
    def _train(self, *xs, **kwargs):
        ...
        pmasks = F.interpolate(masks[:, 0, :, :][None], size=p.shape[-2:]).to(torch.bool)[0] # B 12 20
        pos    = self.position_embedding(p, pmasks) # B 32 12 20

        hs, _, weights  = self.transformer(self.input_proj(p), pmasks, self.query_embed.weight, pos)
        # hs 2 16 7 32 weights 16 240 240
        output_class    = self.class_embed(hs) # 2 16 7 3
        # we only need the output[-1] as output
        output_specific = self.specific_embed(hs)# 2 16 7 4
        output_shared   = self.shared_embed(hs)# 2 16 7 4
        output_shared   = torch.mean(output_shared, dim=-2, keepdim=True)
        output_shared   = output_shared.repeat(1, 1, output_specific.shape[2], 1)# 2 16 7 4
        output_specific = torch.cat([output_specific[:, :, :, :2], output_shared, output_specific[:, :, :, 2:]], dim=-1)# 2 16 7 8
        if self.curve_predictor:
            output_rr = self.curve_embed(hs)
            out = {'pred_logits': output_class[-1], 'pred_curves': output_specific[-1], 'rr': output_rr[-1]}
        else:
...
class AELoss(nn.Module):
    def __init__(self,
        ...
        weight_dict = {'loss_ce': 3, 'loss_curves': 5, 'loss_lowers': 2, 'loss_uppers': 2, 'loss_rr': system_configs.rr_weight}
        ...
        losses  = ['labels', 'curves', 'cardinality', 'rr']
    ...
    def forward(self,
        ...
        loss_dict, indices = self.criterion(outputs, gt_cluxy, rr)
