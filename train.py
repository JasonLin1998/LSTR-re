...
def parse_args():
    ...
    parser.add_argument("--log_loss", default=500)
...
def train(training_dbs, validation_db, start_iter=0, freeze=False, args=None):
    ...
    with open(save_path + 'train_loss.log', 'w') as file:
        file.write('iter\tloss\tclass_error\tlr\n')
    ...
    with stdout_to_tqdm() as save_stdout:
        for iteration in metric_logger.log_every(tqdm(range(start_iter + 1, max_iteration + 1),
            ...
            # training [xs, ys] xs:[2, 16,3,360,640] ys[17, 16,3,115]
            viz_split = 'train'
            ...
            if iteration % args.log_loss == 0:
                with open(save_path + 'train_loss.log', 'a') as file:
                    file.write('{}\t{}\t{}\t{}\n'.format(iteration, loss_value,
                                                                loss_dict_reduced['class_error'], learning_rate))
            del set_loss
    ...
    # sending signal to kill the thread i
    training_pin_semaphore.release()
    validation_pin_semaphore.release()

    # terminating data fetching processes i
