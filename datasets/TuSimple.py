...
class TUSIMPLE(DETECTION):
    def __init__(self, db_config, split):
        ...
        self.root = os.path.join(data_dir)
    ...
    def __getitem__(self, idx, transform=False):
        ...
        lighting = self.configs["lighting"]
        rand_color = self.configs["rand_color"]

        item = self._annotations[idx]
        img = cv2.imread(item['path'])
        mask = np.ones((1, img.shape[0], img.shape[1], 1), dtype=np.bool)
        label = item['label']
        rrs = torch.zeros(5, dtype=torch.float32)
        # to compute r2
        i = 0
        for lane in item['old_anno']['lanes']:
            ys = np.array([r[1] for r in lane])
            xs = np.array([r[0] for r in lane])
            if len(xs) <= 3:
                i = i + 1
                continue
            coffee = np.polyfit(ys, xs, deg=1)
            xs_hat = coffee[0] * ys + coffee[1]
            SSE = np.sum(np.power((xs - xs_hat), 2))
            SST = np.sum(np.power(xs - np.mean(xs), 2))
            rr = 1 - SSE / SST
            rrs[i] = rr
            i = i + 1
        ...
        if transform:
            ...
            label = self._transform_annotation(new_anno, img_wh=(input_size[1], input_size[0]))['label']
        ...
        if rand_color:
            ...
            if lighting:
                lighting_(data_rng, img, 0.1, self.eig_val, self.eig_vec)

        if self.normalize:
            normalize_(img, self.mean, self.std)
        img = img.transpose((2, 0, 1))
        img = torch.from_numpy(img.astype(np.float32))
        mask = torch.from_numpy(np.logical_not(mask[:, :, :, 0]).astype(np.float32))
        gt_lane = torch.from_numpy(label.astype(np.float32))

        return (img, gt_lane, mask, idx, rrs)
