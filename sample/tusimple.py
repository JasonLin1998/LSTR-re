...
def kp_detection(db, k_ind):
    ...
    # images   = torch.zeros((batch_size, 3, input_size[0], input_size[1]), dtype=float) # b, 3, H, W
    # masks    = torch.zeros((batch_size, 1, input_size[0], input_size[1]), dtype=float)  # b, 1, H, W
    gt_lanes = []
    path = []
    ...
    for b_ind in range(batch_size):
        ...
        # images[b_ind] = torch.from_numpy(img.transpose((2, 0, 1)))
        # masks[b_ind] = torch.from_numpy(np.logical_not(mask[:, :, :, 0]))
